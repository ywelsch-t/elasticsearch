...
public class IndexFieldTerm implements Iterable<TermPosition> {
    ...
    private int getLucenePositionsFlags(int flags) {
        int lucenePositionsFlags = (flags & IndexLookup.FLAG_PAYLOADS) > 0 ? PostingsEnum.FLAG_PAYLOADS : 0x0;
        lucenePositionsFlags |= (flags & IndexLookup.FLAG_OFFSETS) > 0 ? PostingsEnum.FLAG_OFFSETS : 0x0;
        return lucenePositionsFlags;
    }

    // get the docs+freqs-only PostingsEnum from the reader.
